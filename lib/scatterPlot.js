...
function scatterPlot(
  ...
  if (xLabels) {
    graph.push("".padStart(xOffset, " ") + `-${xGuide}-`.repeat(width + 1));

    const line = ["".padStart(xOffset, " ")];
    const xLabelLength = maxX.toString().length;
    const xLabelDistance = Math.ceil(
      width / xLabelCount >= 1 ? width / xLabelCount : 1
    );
    let x = minX;
    while (x <= maxX) {
      line.push(` ${x} `.padEnd(xLabelDistance * 3));
      x += xLabelDistance;
    }
    graph.push(line.join(""));
  } else {
    graph.push("".padStart(xOffset, " ") + "".padEnd(width * 3 + 1, "-"));
  }
